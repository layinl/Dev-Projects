...
cadastrarAutomaticamente() {
	...
	for (i=0; i<cadastrosAutomaticos; i++) {
		...
		strcpy(prodDesc[lastCodeGenerated], strcat(desc, string));
		strcpy(prodDept[lastCodeGenerated], strcat(dept, string));
		prodInvCount[lastCodeGenerated] = (rand() % cadastrosAutomaticos * 10);
		prodInvMinCountWarn[lastCodeGenerated] = (rand() % (cadastrosAutomaticos * 10)) / 2;
		prodUnitPrice[lastCodeGenerated] = (rand() % 100);
		lastCodeGenerated++;
		saveLastCodeGenerated = lastCodeGenerated;
	}
	if (i>0) {
		printf("\t\t\t\t (!) Efetuados > %d < cadastros automáticos com sucesso. (!)\n", i);
	}
}
...
debuG(){
	...
	while (userInput != 'z') {
		printf("\n\n\n#\t#\t#\t#\t#\t#\t#\t#\t#\t#\t#\t#\t#\t#\t#\n");
		printf("\n\t\t\t\t\tD E B U G:");
		printf("\n\t\t saveLastCodeGenerated: %d", saveLastCodeGenerated);
		printf("\n\t\t lastCodeGenerated: %d", lastCodeGenerated);
		printf("\n\t\t proximoLugarNaLixeira: %d", proximoLugarNaLixeira);
		printf("\n\t\t cadastrosAutomaticos: %d", cadastrosAutomaticos);
		printf("\n");
		printf("\n\t\t1) prodCode");
		printf("\n\t\t2) prodDesc");
		printf("\n\t\t3) prodDept");
		printf("\n\t\t4) prodInvCount");
		printf("\n\t\t5) prodInvMinCountWarn");
		printf("\n\t\t6) prodUnitPrice");
		printf("\n\t\t7) registrosVazios");
		printf("\n\t\t8) ");
		printf("\n\t\t9) ");
...
fflush(stdin);
		...
		switch (userInput) {
			...
			case '2':
				for (i=0; i<lastCodeGenerated; i++){
					printf("prodDesc[%d]: %s", i, prodDesc[i]);
				}
				break;
			case '3':
				for (i=0; i<lastCodeGenerated; i++){
					printf("prodDept[%d]: %s", i, prodDept[i]);
				}
				break;
			case '4':
				for (i=0; i<lastCodeGenerated; i++){
					printf("prodInvCount[%d]: %d", i, prodInvCount[i]);
				}
				break;
			case '5':
				for (i=0; i<lastCodeGenerated; i++){
					printf("prodInvMinCountWarn[%d]: %d", i, prodInvMinCountWarn[i]);
				}
				break;
			case '6':
				for (i=0; i<lastCodeGenerated; i++){
					printf("prodUnitPrice[%d]: %f", i, prodUnitPrice[i]);
				}
				break;
			case '7':
				for (i=0; i<lastCodeGenerated; i++){
					printf("codProdutosExcluidos[%d]: %d", i, codProdutosExcluidos[i]);
				}
				break;
			case '8':
				break;
